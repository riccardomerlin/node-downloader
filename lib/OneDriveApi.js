...
const folder = 'Pictures/Mobile_uploads';
...
class OneDriveApi {
  ...
  async getFiles(link) {
    const url = link || `https://graph.microsoft.com/v1.0/me/drive/root:/${folder}:/children?select=id,name,size,folder&top=10`; // Pictures/Rullino:/children?select=id,name,size&top=10';
    let result;
    try {
  ...
  async getFile(fileName) {
    ...
    try {
      const response = await axios({
        method: 'get',
        url: `https://graph.microsoft.com/v1.0/me/drive/root:/${folder}/${fileName}:/content`, // :/Pictures/Rullino/${fileName}:/content`,
        responseType: 'stream',
        headers: {
  ...
  async getFolderInfo() {
    ...
    try {
      response = await request
        .get(`https://graph.microsoft.com/v1.0/me/drive/root:/${folder}:/`) // :/Pictures/Rullino:/')
        .set('Authorization', `Bearer ${this.accessToken}`)
        .set('Content-Type', 'application/json');
    } catch (error) {
